...
class Domino:
    ...
    def start(self):
        # Setando váriaveis
        self.model = load_model('domino.h5')
        self.field = []
        self.hand0 = []
        self.hand1 = []
        self.edges = []
        ...
        if len(self.hand0) == 0:
            ...
            if self.valid():
                ...
                if self.round%2 == 0:
                    ...
                    # Verificar qual peça a IA acha melhor
                    # ARRUMAR
                    for ii in range(len(self.aux)):
                        for j in range(len(self.edges)):
                            if self.aux[ii][0] == self.edges[j]:
                                edge = self.edges
                                field = self.field
                                edge[j] = self.aux[0]
                                # Adiciona a nova peça no campo
                                field.append(self.aux[0])
                                hand0 = self.hand1
                                hand1 = self.hand0
                                # Remove a peça da mão
                                del hand0[hand0.index(self.aux[0])]
                                teste.append(np.array(IA(field, edge, hand0, hand1)))
                            
                            if self.aux[ii][1] == self.edges[j]:
                                # Substitui a borda antiga pela nova
